...
導入後に `/aidd-dotnet:init [lite|full|full-pm]`(+ aidd-smart 利用時は `/aidd-smart:init`)を実行する。**展開されるのは `.claude/rules/` のみ**(アーキ規範 rules 39 本 + プロジェクト宣言 aidd.md = SDD レベル)。規範 rules は managed(init 再実行で上書き更新)で、対象ファイルを読むと `paths:` で自動適用される。AGENTS.md / README / ビルド設定はアプリ側で用意し、docs 骨格(adr / work / spec / reference / pm)は各フロー skill が必要時に生成する。
...
更新後は **init を再実行**して規範 rules を最新化する(`/aidd-dotnet:init` + smart 利用時 `/aidd-smart:init`。上書きされるのは `.claude/rules/` の managed ファイルだけで、他には触れない)。フロー skill・hooks・MCP・references はプラグイン本体から直接提供されるため update だけで最新になる。
...
| `plugins/aidd-dotnet/` | 第 1 段の実体(`.claude/rules/` 20 / フロー skills 21 / agents 4 / hooks / MCP / init) |
| `plugins/aidd-smart/` | 第 2 段の実体(`.claude/rules/` 19 / references の器 skills 19 + references 69 / init) |
