...
> **Worker Service (Generic Host の常駐バックグラウンドサービス) 固有**。.NET 共通の規範は対象ファイルを読むと自動適用される。プロジェクト方針は [`conventions.md`](conventions.md) を参照。
...
## 実行モデル
- `BackgroundService.ExecuteAsync(CancellationToken)` を基本に、**CancellationToken を末端まで伝播**して graceful shutdown に応える ([async.md](async.md))。
...
- ORM / データアクセス方式は用途で選定 ([data.md](data.md))。接続文字列は `appsettings` / 環境変数。
