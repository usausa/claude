...
4. frontmatter に `tags` (1〜3 個。語彙は `.claude/rules/` のファイル名を第一候補に、足りなければ `docs/glossary.md`) と `related` (関連 ADR / SPEC) を付ける
...
6. **過去の ADR は絶対に編集しない** (追記のみ。メタデータ行の追加・更新のみ許容)。決定を覆す場合は:
   ...
   - 覆した決定に関連する `.claude/rules/` の見直しも検討する (該当タグの rule を確認)
