---
name: worker
description: Worker / 常駐サービスのアーキテクチャ規約
paths:
  - "src/**"
---
...
## 実行モデル (常駐型)
- `BackgroundService.ExecuteAsync(CancellationToken)` を基本に、**CancellationToken を末端まで伝播**して graceful shutdown に応える (`async` skill)。
...
- ORM / データアクセス方式は用途で選定 (`data` skill)。接続文字列は `appsettings` / 環境変数。

## セキュリティの具体 (`security` skill の実装)
