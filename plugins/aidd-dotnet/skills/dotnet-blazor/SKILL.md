---
name: dotnet-blazor
description: Blazor UI / コンポーネントの規約
paths:
  - "**/*.razor"
  - "**/*.razor.cs"
  - "**/Components/**"
...
> ASP.NET Core Web の **Blazor UI** 固有 (Server / WASM 共通。Server 固有の点はその旨明記)。Web 全般は `dotnet-web` skill、API は `dotnet-api` skill を参照。ホスティング構成・認証基盤・エラーページの配線は起動の組み立て側が担保する。
...
- 桁・書式は Domain の定数 (`Length` / `Pattern`) を参照し、**二重定義しない** (`dotnet-domain` skill)。
...
## セキュリティの具体 (`dotnet-security` skill の実装 / UI 側)
