...
> Windows デスクトップ環境の**共通固有事項** (WPF / WinUI で共有)。MVVM レイヤ・UI 原則は `dotnet-mvvm` skill、UI 技術固有は `dotnet-wpf` skill (将来 `winui`)、プロジェクト方針は プロジェクトの `.claude/rules/conventions.md` を参照。
