...
> **ASP.NET Core (Web) の全般** (API / Blazor 共通)。API 固有は `api` skill、Blazor UI は `blazor` skill。.NET 共通の規範 (async / errors / logging / data / security / http-client) は対象ファイルを読むと自動適用される。プロジェクト方針は プロジェクトの `.claude/rules/conventions.md`。
...
| Endpoints | minimal API (採用時。詳細は `api` skill) |
| Components | Blazor UI (採用時。詳細は `blazor` skill) |
...
## ログの具体 (`logging` skill の実装)
- ロガーの構成は起動の組み立て側に集約し、シンク・レベルは設定ファイルへ委譲する (プロバイダの選定は `/adr`)。出力の書き方は `logging` skill (LoggerMessage 全面採用・配置は適宜分割)。

## 観測性の具体 (`logging` skill の計装節の実装)
- 自動計装 (HTTP / DB) とエクスポートの構成は起動の組み立て側が担保する。機能側で足すのは業務単位のスパンと業務量のメトリクスだけ。

## データの具体 (`data` skill の実装)
- ORM / データアクセス方式は用途で選定 (`data` skill)。接続文字列は `appsettings` / 環境変数 (`GetConnectionString()`)。

## セキュリティの具体 (`security` skill の実装 / サーバ側)
...
- 非 GET の状態変更は antiforgery/CSRF 対策 (Blazor 側は `blazor` skill)。
