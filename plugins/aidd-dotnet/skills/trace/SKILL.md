...
1. `docs/spec/`・`docs/adr/` の `id` と参照関係を収集
2. 次を検出して表で報告:
   - `related` に書かれた ID が実在しない(**リンク切れ**)
   - どこからも参照されない**孤児** `SPEC-`
   - **退役候補**: SPEC が参照していた code / test が消えている (= 機能が削除された) → `status: retired` へ (または SPEC 削除)。ADR は不変なので `status: superseded/retired` にし理由を残す
