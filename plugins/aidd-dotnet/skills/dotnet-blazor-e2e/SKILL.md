---
name: dotnet-blazor-e2e
description: Blazor E2E テスト (Playwright) の手順と規約
paths:
  - "tests/**/*E2ETests*/**"
  - "**/playwright.config.*"
---
...
> 配置は `tests/README.md`、UI 原則は `dotnet-blazor` skill に従う。E2E はクリティカルパス (ログイン・主要フロー) に絞り、ロジックの網羅は下位層のユニットテストへ寄せる (`csharp-layered-feature` 参照)。
