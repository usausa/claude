...
## 実行モデル (常駐型)
- `BackgroundService.ExecuteAsync(CancellationToken)` を基本に、**CancellationToken を末端まで伝播**して graceful shutdown に応える (`dotnet-async` skill)。
...
- ORM / データアクセス方式は用途で選定 (`dotnet-data` skill)。接続文字列は `appsettings` / 環境変数。

## セキュリティの具体 (`dotnet-security` skill の実装)
