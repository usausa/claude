...
> ASP.NET Core Web の **Blazor UI** 固有 (Server / WASM 共通。Server 固有の点はその旨明記)。Web 全般は `dotnet-web` rule、API は `dotnet-api` rule を参照。ホスティング構成・認証基盤・エラーページの配線は起動の組み立て側が担保する。
...
- 桁・書式は Domain の定数 (`Length` / `Pattern`) を参照し、**二重定義しない** (`dotnet-domain` rule)。
...
## セキュリティの具体 (`dotnet-security` rule の実装 / UI 側)
