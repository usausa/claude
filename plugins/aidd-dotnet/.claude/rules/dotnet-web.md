...
> **ASP.NET Core (Web) の全般** (API / Blazor 共通)。API 固有は `dotnet-api` rule、Blazor UI は `dotnet-blazor` rule。.NET 共通の規範 (async / errors / logging / data / security / http-client) は対象ファイルを読むと自動適用される。プロジェクト方針は プロジェクトの `.claude/rules/conventions.md`。
...
| Endpoints | minimal API (採用時。詳細は `dotnet-api` rule) |
| Components | Blazor UI (採用時。詳細は `dotnet-blazor` rule) |
...
- ロガーの構成は起動の組み立て側に集約し、シンク・レベルは設定ファイルへ委譲する (プロバイダの選定は `/adr`)。出力の書き方は `dotnet-logging` rule (LoggerMessage 全面採用・配置は適宜分割)。
...
- ORM / データアクセス方式は用途で選定 (`dotnet-data` rule)。接続文字列は `appsettings` / 環境変数 (`GetConnectionString()`)。

## セキュリティの具体 (`dotnet-security` rule の実装 / サーバ側)
...
- 非 GET の状態変更は antiforgery/CSRF 対策 (Blazor 側は `dotnet-blazor` rule)。
