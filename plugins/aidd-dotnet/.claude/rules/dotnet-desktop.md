...
> Windows デスクトップ環境の**共通固有事項** (WPF / WinUI で共有)。MVVM レイヤ・UI 原則は `dotnet-mvvm` rule、UI 技術固有は `dotnet-wpf` rule (将来 `winui`)、プロジェクト方針は プロジェクトの `.claude/rules/conventions.md` を参照。
